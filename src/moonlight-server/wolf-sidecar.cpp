...
void run() {
  ...
  if (!receiver_ip || !receiver_video_port || !receiver_audio_port) {
    logs::log(logs::error, "WOLF_RECEIVER_HOST, WOLF_RECEIVER_VIDEO_PORT and WOLF_RECEIVER_AUDIO_PORT must be set");
    std::exit(1);
  }
